...
# Create the host account before agent setup, matching the other agent images.
: "${HOME:=/tmp/home}"
: "${XDG_CONFIG_HOME:=$HOME/.config}"
: "${XDG_CACHE_HOME:=$HOME/.cache}"
: "${XDG_DATA_HOME:=$HOME/.local/share}"
: "${CODEX_HOME:=/var/lib/codex}"
export HOME XDG_CONFIG_HOME XDG_CACHE_HOME XDG_DATA_HOME CODEX_HOME
...
# This directory is ephemeral, outside the persistent CODEX_HOME bind mount.
# Rebuild on entry so deleted or renamed OpenCode prompts cannot linger.
skills_dir="$HOME/.agents/skills"
mkdir -p "$HOME/.agents"
if [ -d "$skills_dir" ]; then
	rm -rf -- "$skills_dir"
fi
python3 /usr/local/lib/cod/integrate-config.py --destination "$skills_dir"
