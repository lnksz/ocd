...
Your existing OpenCode login (`auth.json` in the data directory), config,
plugins, agents, commands, and skills are shared automatically, just as with
`ocd`. Existing `config.env` is passed using `sbx exec --env-file`; it is not
sourced as shell code. Host config changes take effect when you quit and
restart OpenCode with `ox`.
...
`ox --rm` uses the normal `sbx rm` behavior: an active session prevents removal.
Exit the session first. Files in shared host directories survive removal;
packages, Docker images, and files stored only inside the sandbox do not.
...
| OpenCode config, cache, data and login | Same host XDG `opencode/` directories, linked into the sandbox's native HOME |
...
| Lifetime | Persistent per working directory, including installed tools and shell history |
...
not guarantee a fresh `latest` image.
...
files, OpenCode history, configuration and login survive; sandbox-only packages,
Docker images and files are removed with the old sandbox.
...
variables are passed anew on every invocation.
...
- Sharing host `auth.json`, auth directories and `config.env` gives the agent
  access to those credentials, as `ocd` does. It does **not** convert them into
  proxy-protected secrets. Docker also supports `sbx secret set` for supported
  providers; see its [credential guide](https://docs.docker.com/ai/sandboxes/configuration/credentials/).
...
stores TUI preferences in the sandbox, separately from the shared OpenCode data
...
container engine. The base image manifest and real sbx 0.42.1 CLI help were also
inspected. **Image build and live microVM/TUI/provider behavior have not been
verified in the development container**, which has neither Docker nor KVM.
